...
def load_wsj_from_ldc(ldc_dir, split="train"):
    ...
    if split == "train":
        ...
        raise ValueError("split train/dev/test")
...
def collect_hidden_states(model, parsed, encoded, layers):
    ...
    _, _ = model.set_chunk_representations(input_ids, parsed, None)
    ...
    for idx, helper in model._cross_attn_helpers.items():
        helper.chunk_tensor = None
        helper.token_mask = None
...
def train_probe_on_ptb(model, tokenizer, layers, proj_dim, n_train=3000, lr=1e-3, epochs=3, outputs="probe_state.pt"):
    ...
    probe = MultiLayerStructuralProbe(model.model.qwen.config.hidden_size, layers, proj_dim)
    ...
    for l in layers:
        scale_l = estimate_initial_scale(model.model, tokenizer, sample_tree, l)
    ...
    for ep in range(epochs):
        ...
        loss_layer = {l: [] for l in layers}
        
        for batch in pbar:
            parsed, encoded, tree = batch["parsed"], batch["encoded"], batch["tree"]
            
            QwenParseTreeModel = model.model
            for idx, helper in QwenParseTreeModel._cross_attn_helpers.items():
                helper.alpha = 0.15
            hs = collect_hidden_states(QwenParseTreeModel, [{"tokens": batch["sent"], "tree_structure": parsed}], batch["encoded"], layers)
            total_loss = 0.0
            rank = dist.get_rank() if dist.is_initialized() else 0
            ...
            for l in layers:
                ...
                gold_vals = gold_vals / (gold_vals.mean() + 1e-6)
                pred_vals = pred_vals / (pred_vals.mean() + 1e-6)


                loss = F.mse_loss(pred_vals, gold_vals)

                total_loss += loss
                loss_layer[l].append(loss.item())
                ...
                if mat.shape[0] >= 2:
                    ...
                    uuas = compute_uuas(mat, tree, batch["poses"], bias=0.45)
        ...
        print(f"Epoch {ep}: loss={epoch_avg_loss:.4f}, lr={optimizers[str(0)].param_groups[0]['lr']:.6f}")
...
def ckpt_sort_key(p: Path):
    if p.is_file() and p.suffix == ".ckpt":
        return extract_epoch(p.name)
    return extract_epoch(p.parent.name)

def find_stage_ckpts(d: Path):
    candidates = []

    candidates += [p for p in d.rglob("*.ckpt") if p.is_file()]

    candidates += list(d.rglob("*.ckpt/mp_rank_00_model_states.pt"))
    candidates += list(d.rglob("mp_rank_00_model_states.pt"))
    candidates = sorted(set(candidates), key=ckpt_sort_key)
    return candidates
...
def main():
    ...
    ckpt_dirs = [Path(p) for p in args.ckpt_dirs.split(",")]

    hparams_parsetree = Hparams_parsetree(
            args.auxloss_coefficient, 
            args.layers_ratio, 
            args.heads_ratio, 
            args.reweight_coefficient
        )
    
    tokenizer = AutoTokenizer.from_pretrained(args.model_name, trust_remote_code=True)

    model = LitQwenParseTreeModel(
            learning_rate=5e-5,
    ...
    for idx, helper in model.model._cross_attn_helpers.items():
        helper.alpha = args.reweight_coefficient
    

    # ========= train =========
    if args.stage == "train":
        
        best_ckpt = Path(args.best_ckpts) 

        sd = torch.load(best_ckpt, map_location="cpu")
        model.load_state_dict(sd.get("module", sd), strict=False)
        del sd
        torch.cuda.empty_cache()            
        model.to(device)

        layers = list(range(model.model.qwen.config.num_hidden_layers))
        ...
        probe = MultiLayerStructuralProbe(model.model.qwen.config.hidden_size, layers, args.proj_dim)
        ...
        all_rows = []
        for stage_i, d in enumerate(ckpt_dirs, 1):
            stage_ckpts = find_stage_ckpts(d)
            if len(stage_ckpts) > 10:
                stage_ckpts = stage_ckpts[:10]  
            
            stage_res = {}
            for ck in tqdm(stage_ckpts, desc=f"Stage {stage_i}"):
                print(f"Evaluating checkpoint: {ck.name}")
               
                sd = torch.load(ck, map_location="cpu")
                if "state_dict" in sd:
                    res = model.load_state_dict(sd.get("state_dict", sd), strict=False)
                else:
                    res = model.load_state_dict(sd.get("module", sd), strict=False)
                ...
                stage_res[ck.name] = avg
                
                
            json_path = f"metrics_stage{stage_i}.json"
            with open(json_path, "w") as f:
                json.dump(stage_res, f, indent=2)
            csv_path = f"metrics_stage{stage_i}.csv"
            rows = write_stage_csv(f"stage{stage_i}", stage_res, csv_path)
            all_rows.extend(rows)
       
            print(f" Stage {stage_i} done.")


        all_csv = "metrics_all_stages.csv"
        if all_rows:
            with open(all_csv, "w", newline="", encoding="utf-8") as f:
                writer = csv.DictWriter(f, fieldnames=all_rows[0].keys())
                writer.writeheader()
                writer.writerows(all_rows)
            print(f" All stages CSV saved to {all_csv}")
